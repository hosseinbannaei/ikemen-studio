<script lang="ts">
  import { onMount } from 'svelte';
  import { engineStore } from '../stores/engineStore';
  import {
    X,
    Download,
    Trash2,
    RefreshCw,
    CheckCircle2,
    Layers,
    ExternalLink,
    ChevronDown,
    ChevronUp,
    HardDrive,
    XCircle,
    Loader2,
  } from 'lucide-svelte';

  export let onClose: () => void;

  let activeTab: 'installed' | 'available' = 'installed';
  let filterChannel: 'all' | 'stable' | 'nightly' = 'all';
  let expandedRelease: string | null = null;

  onMount(() => {
    engineStore.loadInstalled();
    engineStore.loadAvailable();
  });

  function isInstalled(tag: string): boolean {
    const download = $engineStore.downloads[tag];
    if (download && (download.status === 'downloading' || download.status === 'extracting')) {
      return false;
    }
    return $engineStore.installed.some(
      (e) => e.version.toLowerCase() === tag.toLowerCase()
    );
  }

  function formatBytes(bytes: number): string {
    if (!bytes || bytes === 0) return '0 B';
    const k = 1024;
    const sizes = ['B', 'KB', 'MB', 'GB'];
    const i = Math.floor(Math.log(bytes) / Math.log(k));
    return parseFloat((bytes / Math.pow(k, i)).toFixed(1)) + ' ' + sizes[i];
  }

  function formatDate(dateStr: string): string {
    if (!dateStr) return '';
    try {
      return new Date(dateStr).toLocaleDateString(undefined, {
        year: 'numeric',
        month: 'short',
        day: 'numeric',
      });
    } catch {
      return dateStr;
    }
  }

  $: filteredAvailable = $engineStore.available.filter((rel) => {
    if (filterChannel === 'stable') return !rel.isPrerelease && !rel.tag.toLowerCase().includes('nightly');
    if (filterChannel === 'nightly') return rel.isPrerelease || rel.tag.toLowerCase().includes('nightly');
    return true;
  });
</script>

<div class="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
  <div class="bg-dark-800 border border-dark-600/80 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
    <!-- Header -->
    <div class="p-5 border-b border-dark-600/60 flex items-center justify-between bg-dark-800/80">
      <div class="flex items-center gap-3">
        <div class="w-9 h-9 rounded-xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-indigo-400">
          <Layers class="w-5 h-5" />
        </div>
        <div>
          <h2 class="text-base font-bold text-slate-100">Engine Version Manager</h2>
          <p class="text-xs text-slate-400">Manage downloaded Ikemen GO engine binaries & GitHub releases</p>
        </div>
      </div>
      <div class="flex items-center gap-2">
        <button
          type="button"
          title="Refresh Releases"
          class="p-2 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-dark-700 transition"
          on:click={() => {
            engineStore.loadInstalled();
            engineStore.loadAvailable();
          }}
        >
          <RefreshCw class="w-4 h-4 {$engineStore.loadingAvailable || $engineStore.loadingInstalled ? 'animate-spin text-indigo-400' : ''}" />
        </button>
        <button
          type="button"
          class="p-2 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-dark-700 transition"
          on:click={onClose}
        >
          <X class="w-5 h-5" />
        </button>
      </div>
    </div>

    <!-- Subheader / Tabs -->
    <div class="px-5 pt-3 border-b border-dark-600/40 flex items-center justify-between bg-dark-900/40">
      <div class="flex gap-4">
        <button
          type="button"
          class="pb-3 text-xs font-semibold tracking-wide border-b-2 transition {
            activeTab === 'installed'
              ? 'border-indigo-500 text-indigo-400'
              : 'border-transparent text-slate-400 hover:text-slate-300'
          }"
          on:click={() => (activeTab = 'installed')}
        >
          Installed Versions ({$engineStore.installed.length})
        </button>
        <button
          type="button"
          class="pb-3 text-xs font-semibold tracking-wide border-b-2 transition {
            activeTab === 'available'
              ? 'border-indigo-500 text-indigo-400'
              : 'border-transparent text-slate-400 hover:text-slate-300'
          }"
          on:click={() => (activeTab = 'available')}
        >
          Available Releases ({$engineStore.available.length})
        </button>
      </div>

      {#if activeTab === 'available'}
        <div class="flex items-center gap-1.5 pb-2">
          <button
            type="button"
            class="px-2.5 py-1 text-[11px] font-medium rounded-md transition {filterChannel === 'all' ? 'bg-indigo-600 text-white' : 'bg-dark-700 text-slate-400 hover:text-slate-200'}"
            on:click={() => (filterChannel = 'all')}
          >
            All
          </button>
          <button
            type="button"
            class="px-2.5 py-1 text-[11px] font-medium rounded-md transition {filterChannel === 'stable' ? 'bg-indigo-600 text-white' : 'bg-dark-700 text-slate-400 hover:text-slate-200'}"
            on:click={() => (filterChannel = 'stable')}
          >
            Stable
          </button>
          <button
            type="button"
            class="px-2.5 py-1 text-[11px] font-medium rounded-md transition {filterChannel === 'nightly' ? 'bg-indigo-600 text-white' : 'bg-dark-700 text-slate-400 hover:text-slate-200'}"
            on:click={() => (filterChannel = 'nightly')}
          >
            Nightly / Pre
          </button>
        </div>
      {/if}
    </div>

    <!-- Tab Contents -->
    <div class="p-5 flex-1 overflow-y-auto space-y-3">
      {#if activeTab === 'installed'}
        {#if $engineStore.loadingInstalled && $engineStore.installed.length === 0}
          <div class="py-16 text-center text-slate-400 flex flex-col items-center gap-2">
            <RefreshCw class="w-6 h-6 animate-spin text-indigo-400" />
            <span class="text-sm">Scanning engines directory...</span>
          </div>
        {:else if $engineStore.installed.length === 0}
          <div class="py-16 text-center text-slate-400 flex flex-col items-center gap-3">
            <HardDrive class="w-10 h-10 text-slate-600" />
            <div>
              <div class="text-sm font-semibold text-slate-300">No engines installed yet</div>
              <div class="text-xs text-slate-500 mt-1">Switch to the "Available Releases" tab to download an Ikemen GO version.</div>
            </div>
            <button
              type="button"
              class="mt-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-lg shadow-sm transition"
              on:click={() => (activeTab = 'available')}
            >
              Browse Releases
            </button>
          </div>
        {:else}
          {#each $engineStore.installed as engine (engine.version)}
            <div class="p-4 rounded-xl bg-dark-900/60 border border-dark-600/50 flex items-center justify-between hover:border-dark-600 transition">
              <div class="space-y-1">
                <div class="flex items-center gap-2.5">
                  <span class="font-bold text-sm text-slate-100">{engine.version}</span>
                  <span
                    class="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full {
                      engine.channel === 'stable'
                        ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                        : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                    }"
                  >
                    {engine.channel}
                  </span>
                  {#if engine.executablePath}
                    <span class="text-[10px] text-emerald-400 flex items-center gap-1 font-medium">
                      <CheckCircle2 class="w-3 h-3" /> Ready
                    </span>
                  {/if}
                </div>
                <div class="text-xs text-slate-400 font-mono flex items-center gap-3">
                  <span>Size: {formatBytes(engine.size)}</span>
                  <span>•</span>
                  <span>Path: {engine.path}</span>
                </div>
              </div>

              <div class="flex items-center gap-2">
                <button
                  type="button"
                  class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-rose-400 hover:bg-rose-950/40 hover:border-rose-800/50 border border-transparent transition"
                  on:click={() => {
                    if (confirm(`Are you sure you want to delete engine version ${engine.version}?`)) {
                      engineStore.removeEngine(engine.version);
                    }
                  }}
                >
                  <Trash2 class="w-3.5 h-3.5" />
                  Delete
                </button>
              </div>
            </div>
          {/each}
        {/if}
      {:else}
        <!-- Available Releases Tab -->
        {#if $engineStore.loadingAvailable && $engineStore.available.length === 0}
          <div class="py-16 text-center text-slate-400 flex flex-col items-center gap-2">
            <RefreshCw class="w-6 h-6 animate-spin text-indigo-400" />
            <span class="text-sm">Fetching releases from GitHub...</span>
          </div>
        {:else if filteredAvailable.length === 0}
          <div class="py-16 text-center text-slate-400">No releases found matching the filter.</div>
        {:else}
          {#each filteredAvailable as release (release.tag)}
            {@const download = $engineStore.downloads[release.tag]}
            {@const isDownloading = download && (download.status === 'downloading' || download.status === 'extracting')}
            {@const installed = isInstalled(release.tag)}
            <div class="p-4 rounded-xl bg-dark-900/60 border border-dark-600/50 space-y-3">
              <div class="flex items-start justify-between gap-4">
                <div class="space-y-1">
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="font-bold text-sm text-slate-100">{release.tag}</span>
                    {#if release.name && release.name !== release.tag}
                      <span class="text-xs text-slate-300 font-medium">{release.name}</span>
                    {/if}
                    <span
                      class="text-[10px] font-mono uppercase px-2 py-0.5 rounded-full {
                        release.isPrerelease || release.tag.toLowerCase().includes('nightly')
                          ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                          : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                      }"
                    >
                      {release.isPrerelease || release.tag.toLowerCase().includes('nightly') ? 'Nightly / Pre' : 'Stable'}
                    </span>
                    {#if installed}
                      <span class="text-[10px] px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-medium">
                        Installed
                      </span>
                    {/if}
                  </div>
                  <div class="text-xs text-slate-400 flex items-center gap-2">
                    <span>Published {formatDate(release.publishedAt)}</span>
                    {#if release.htmlUrl}
                      <span>•</span>
                      <a
                        href={release.htmlUrl}
                        target="_blank"
                        rel="noreferrer"
                        class="text-indigo-400 hover:underline inline-flex items-center gap-1"
                      >
                        GitHub <ExternalLink class="w-3 h-3" />
                      </a>
                    {/if}
                  </div>
                </div>

                <!-- Download / Status Action -->
                <div>
                  {#if isDownloading}
                    <div class="flex items-center gap-2">
                      <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-indigo-950/60 text-indigo-300 text-xs font-medium border border-indigo-700/40">
                        <Loader2 class="w-3.5 h-3.5 animate-spin text-indigo-400" />
                        <span class="capitalize">{download.status}... {Math.round(download.percent)}%</span>
                      </div>
                      <button
                        type="button"
                        class="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-rose-950/60 hover:bg-rose-900 border border-rose-800/40 text-rose-300 text-xs font-medium transition"
                        title="Cancel Download"
                        on:click={() => engineStore.cancelDownload(release.tag)}
                      >
                        <XCircle class="w-3.5 h-3.5" />
                        <span>Cancel</span>
                      </button>
                    </div>
                  {:else if installed}
                    <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-dark-700/60 text-emerald-400 text-xs font-semibold border border-dark-600/40">
                      <CheckCircle2 class="w-4 h-4" /> Ready
                    </div>
                  {:else}
                    <button
                      type="button"
                      class="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow transition"
                      on:click={() => engineStore.startDownload(release.tag)}
                    >
                      <Download class="w-3.5 h-3.5" />
                      Download
                    </button>
                  {/if}
                </div>
              </div>

              <!-- Progress Bar -->
              {#if isDownloading}
                <div class="space-y-1">
                  <div class="w-full bg-dark-700 rounded-full h-1.5 overflow-hidden">
                    <div
                      class="bg-indigo-500 h-1.5 rounded-full transition-all duration-200"
                      style="width: {download.percent}%"
                    ></div>
                  </div>
                  <div class="flex justify-between text-[11px] text-slate-400 font-mono">
                    <span>{download.status === 'downloading' ? 'Downloading engine package...' : 'Extracting & verifying binary...'}</span>
                    <span>{Math.round(download.percent)}%</span>
                  </div>
                </div>
              {/if}

              <!-- Release Notes Toggle -->
              {#if release.body}
                <div>
                  <button
                    type="button"
                    class="text-xs text-slate-400 hover:text-slate-200 flex items-center gap-1"
                    on:click={() => (expandedRelease = expandedRelease === release.tag ? null : release.tag)}
                  >
                    {#if expandedRelease === release.tag}
                      <ChevronUp class="w-3.5 h-3.5" /> Hide Release Notes
                    {:else}
                      <ChevronDown class="w-3.5 h-3.5" /> Show Release Notes
                    {/if}
                  </button>
                  {#if expandedRelease === release.tag}
                    <div class="mt-2 p-3 rounded-lg bg-dark-800 text-xs text-slate-300 max-h-40 overflow-y-auto whitespace-pre-wrap font-mono leading-relaxed border border-dark-600/40">
                      {release.body}
                    </div>
                  {/if}
                </div>
              {/if}
            </div>
          {/each}
        {/if}
      {/if}
    </div>
  </div>
</div>
